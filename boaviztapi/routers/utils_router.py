...
async def name_to_cpu(cpu_name: str = Query(example="Intel Core i7-9700K")):
    ...
    if cpu_attributes is not None:
        name, manufacturer, code_name, model_range, tdp, cores, threads, total_die_size, total_die_size_source, source = cpu_attributes
        return CPU(family=code_name, name=name, tdp=tdp, core_units=cores, threads=threads, die_size=total_die_size, model_range=model_range,
